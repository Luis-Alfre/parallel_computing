...
void generarMatriz(double M[], int f, int c, int max)
{
 ...
 void ordenarMatrizSerial(double M[], int f, int c, int k){
 ...
 void ordenarFilaMatrizParallel(double M[], int f, int c, int k){

    double t1 = omp_get_wtime(); // tiempo inicio
	double aux=0; 
	int s=0;
	int i,j;
	#pragma omp parallel for private(i,j) shared(M)  num_threads(k)
	for (i=0;i<c;i++){
			for (j=0;j<f-1;j++){
				s=j+1;
                if(M[j*f+i]>M[s*f+i]){
 ...
 int main()
 {
	...
	for (j = 1; j < 11; j++){
        printf("\n\n**__** # PROCESADORES = %d \n", j);
        for (x = 1; x < 4; x++){
        ordenarMatrizSerial(M,f,c,j);
		ordenarFilaMatrizParallel(M,f,c,j);
    }
	}
	//printf("\n El vector resultado: \n");
	//imprimirVector(V,f);
