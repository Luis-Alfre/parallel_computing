...
void generarMatriz(double M[], int f, int c, int max)
{
 ...
 void promFilaMatrizSerial(double M[], double V[], int f, int c, int k){
 ...
 void promFilaMatrizParallel(double M[], double V[], int f, int c, int k){
    double t1 = omp_get_wtime(); // tiempo iniciO 
    int sum=0;
	int i,j;
	#pragma omp parallel for private(i,j) shared(M, V) reduction(+:sum) num_threads(k)
	for (i=0;i<f;i++){
        sum=0;
			for (j=0;j<c;j++){
                    sum+=M[i*c+j];
 ...
 int main()
 {
	...
	for (j = 1; j < 11; j++){
        printf("\n\n**__** # PROCESADORES = %d \n", j);
        for (x = 1; x < 4; x++){
        promFilaMatrizSerial(M,V,f,c,j);
		promFilaMatrizParallel(M,V,f,c,j);
    }
	}
	//printf("\n El vector resultado: \n");
	//imprimirVector(V,f);
